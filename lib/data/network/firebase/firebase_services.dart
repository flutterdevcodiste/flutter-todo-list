...
class FirebaseService {
  ...
  static Future<void> createAccount() async {
    try {
      ...
      //     debugPrint(Utils.extractFirebaseError(error.toString()));
      //     Utils.showSnackBar(
      //         'Error',
  ...
  static Future<void> loginAccount() async {
    try {
      ...
      var email = signInController.email.value.text.toString();
      var password = signInController.password.value.text.toString();
  ...
  static Future<void> signInwWithGoogle() async {
    try {
      ...
      //               color: Colors.red,
      //             ));
      //         UserPref.setUser(value.user?.displayName ?? "", value.user?.email ?? "", "NOPASSWORD",
      //             node ?? "", value.user?.uid ?? "");
      //         Get.to(HomePage());
      //       }).onError((error, stackTrace) {
      //         debugPrint(Utils.extractFirebaseError(error.toString()));
  ...
  static Future<void> logoutAccount() async {
    ...
    Get.to(const SplashView(), routeName: "/");
  }
  ...
  //   String str = auth.currentUser!.email.toString();
  //   String node = str.substring(0, str.indexOf('@'));
  //   database.ref('Tasks').child(node).child(key).update({updateKey: updateValue});
  // }
}
